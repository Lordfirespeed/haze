...
public class HazeWebSocket
{
    ...
    public async Task<HazeC2SMessage?> ReceiveMessage(CancellationToken ct = default)
    {
        ...
        catch (JsonException exception)
        {
            ...
            await WebSocket.CloseAsync(WebSocketCloseStatus.PolicyViolation, "Only messages of type 'text' containing a single JSON-encoded object are acceptable", ct);
            return null;
        }
        ...
        if (message is null || message.GetType() == typeof(HazeC2SMessage))
        {
            ...
            await WebSocket.CloseAsync(WebSocketCloseStatus.PolicyViolation, "Empty messages (i.e. 'null', '{}') are not acceptable", ct);
            return null;
        }
    ...
    {
        await using var receiveScope = await _receiveLock.EnterScope();
